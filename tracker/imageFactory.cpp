...
ImageFactory::~ImageFactory() {
  ...
  std::map<std::string, SDL_Surface*>::const_iterator ptrSur =
    surfaces.begin();
  while ( ptrSur != surfaces.end() ) {
    SDL_FreeSurface(ptrSur->second);
    ++ptrSur;
  }

  std::map<std::string, SDL_Texture*>::const_iterator ptrTex =
    textures.begin();
  while ( ptrTex != textures.end() ) {
    SDL_DestroyTexture(ptrTex->second);
    ++ptrTex;
  }
